...
export default function UserChat({ isOpen, onClose }: UserChatProps) {
        ...
        {!selectedConversation ? (
          <>
            <div className="flex items-center justify-between p-4 border-b border-[#363636]">
              ...
              <button onClick={onClose} className="text-[#adadad] hover:text-white transition-colors">
            ...
            <div className="flex-1 overflow-y-auto">
              {conversations.map((conversation) => {
                ...
                return (
                  <div key={conversation.id} onClick={() => setSelectedConversation(conversation)} className="p-3 border-b border-[#2d2d2d] hover:bg-[#2d2d2d] cursor-pointer transition-colors">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[#adadad] text-xs truncate">{conversation.lastMessage?.content || 'No messages yet'}</p>
                        <p className="text-[#adadad] text-xs">{new Date(conversation.updatedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                      </div>
                    </div>
                  </div>
                )
          ...
          <>
            <div className="flex items-center justify-between p-4 border-b border-[#363636]">
              <div className="flex items-center gap-3">
                <button onClick={() => setSelectedConversation(null)} className="text-[#adadad] hover:text-white transition-colors">
                  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" viewBox="0 0 256 256">
                ...
                <div>
                  ...
                  <p className="text-[#adadad] text-xs">{getOtherParticipant(selectedConversation)?.isOnline ? 'Online' : 'Last seen recently'}</p>
                </div>
              </div>
              <button onClick={onClose} className="text-[#adadad] hover:text-white transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" viewBox="0 0 256 256">
                ...
                {getMessagesByConversation(selectedConversation.id).map((message) => {
                  ...
                  return (
                    <div key={message.id} className={`flex gap-3 ${isCurrentUser ? 'justify-end' : 'justify-start'}`}>
                      ...
                      <div className={`max-w-[75%] rounded-lg px-3 py-2 ${isCurrentUser ? 'bg-white text-black' : 'bg-[#2d2d2d] text-white'}`}>
            ...
            <div className="p-4 border-t border-[#363636]">
              <form onSubmit={handleSendMessage} className="flex gap-2">
                ...
                />
                <button type="submit" disabled={!newMessage.trim()} className="px-3 py-2 bg-white hover:bg-gray-100 disabled:bg-[#4d4d4d] disabled:text-[#adadad] text-black rounded-lg transition-colors">
